...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, addr_t size, addr_t *alloc_addr)
{
  ...
  inc_sz = inc_sz + 1;  // remember to increase 1 page
#else
...
// struct sc_regs regs;
  ...
  addr_t tgtfpn;
     ...
     */
    addr_t tgtswpfpn = PAGING_SWP(pte);  // swap frame of needed space
...
int libkmem_copy_from_user(struct pcb_t *caller, uint32_t source, uint32_t destination, uint32_t offset, uint32_t size)
{
  ...
  if ((addr_t)offset + (addr_t)size > src_sz) {
    return -1;
  }
  ...
  if (dst_rg == NULL || (dst_rg->rg_start == 0 && dst_rg->rg_end == 0)) {
      // pthread_mutex_unlock(&mmvm_lock);
      return -1;
  }
  
  addr_t dst_sz = dst_rg->rg_end - dst_rg->rg_start;
  if ((addr_t)size > dst_sz) {
      // pthread_mutex_unlock(&mmvm_lock);
      return -1;
  }
...
int __read_user_mem(struct pcb_t *caller, int vmaid, int rgid, addr_t offset, BYTE *data)
{
  ...
  // current region is not allocated
